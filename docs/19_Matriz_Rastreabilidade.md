...
## 7. Atualizações de status — Sprint 3 (F2/F3/F4/F5 oficiais)

| Linha | Mudança | Sprint / Fatia |
|-------|---------|----------------|
| RF-AMO-001 | `pending` -> `done` com domínio puro F2, service/API/contrato F3, frontend F4 e conteúdo educacional/docs vivos F5. | Sprint 3 F2/F3/F4/F5 |
| RF-AMO-002 | `pending` -> `done` com domínio puro F2, service/API/contrato F3, frontend F4 e conteúdo educacional/docs vivos F5. | Sprint 3 F2/F3/F4/F5 |
| RF-AMO-003 | Linha adicionada e fechada para o comparativo `POST /api/v1/amortization/compare`, incluindo contrato, frontend e conteúdo pedagógico. | Sprint 3 F3/F4/F5 |

Notas:
- A F4 materializou a página `/amortizacao` como frontend real.
- A F5 materializa `frontend/src/content/amortizacao/`, renderiza o conteúdo
  na página e adiciona testes editoriais/runtime para PRICE, SAC, comparação,
