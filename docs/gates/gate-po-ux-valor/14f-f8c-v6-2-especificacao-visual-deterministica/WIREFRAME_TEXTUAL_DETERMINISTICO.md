...
- Respeita o orçamento vertical da viewport (ver Especificação §7).
...
**Layout:**
- Região A (topo, largura total): título "SAC x PRICE — Dois caminhos para amortizar o mesmo valor", fundo `--indigo`, texto branco.
...
- Região D (largura total): ALERTA `--orange`: "Comparar SAC x PRICE de forma justa exige mesma taxa, mesmo prazo e mesmo principal. Taxas diferentes tornam a comparação inválida."
...
- Região C (largura total, fundo `--soft-bg`): CARD-L "Comparação automática: ao calcular, a plataforma sempre compara SAC x PRICE usando a mesma base de parâmetros — garantia de comparação justa."
...
**Layout:**
- Região A (topo): título "SAC x PRICE — Comparação Conceitual e Numérica", fundo `--navy`, texto branco.
- Região B (grid 2 colunas iguais):
...
**Objetivo:**
Exibir a comparação SAC x PRICE de forma sintética e visualmente impactante.

**Layout:**
- Região A (topo): título "SAC x PRICE — Qual é mais barato?", fundo `--deep-blue`, texto branco.
- Região B (grid 2 colunas):
...
**Objetivo:**
Visualizar graficamente as diferenças SAC x PRICE.

**Layout:**
- Região A (topo): título "Comparação Visual SAC x PRICE", fundo `--indigo`, texto branco.
...
**Objetivo:**
Ensinar o usuário a interpretar a comparação SAC x PRICE e fazer a escolha certa.
...
- Valores corretos.
- Tabela cabe sem paginação no orçamento vertical da viewport (ver Especificação §7).
...
- Região B (coluna max-width: 700px, centralizada): CHECKLIST de verificações com status visual (✓ ou ✗):
  ...
  4. Comparação SAC x PRICE usa mesma taxa (0,85%), mesmo prazo (120), mesmo principal (170.000) ✓
...
- Região B (coluna centralizada max-width: 700px):
  - CARD Aurora Gradient Border: "Você completou a jornada de simulação de financiamento imobiliário. Agora você sabe calcular, interpretar e comparar SAC x PRICE com base justa." Fundo `--deep-blue`, Aurora border.
